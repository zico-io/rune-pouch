...
import SuperRanging from '/img/potions/Super ranging.png'
import SuperCombat from '/img/potions/Super combat.png'
import SuperMagic from '/img/potions/Super magic.png'
import Defence from '/img/potions/Defence.png'
import SuperDefence from '/img/potions/Super defence.png'
...
export const fetchPlayerSkills = async (username: string) => {
  const res = await axios.get<string>(
    `${import.meta.env.API_PROXY}/m=hiscore_oldschool/index_lite.ws?player=${username.replaceAll(' ', '_')}`,
  )
...
export const fetchShortlinkData = async (
  linkId: string,
): Promise<ImportableData> => {
  const res = await axios.get<{ data: ImportableData }>(
    `${SHORTLINK_API}?id=${linkId}`,
  )
...
export const getWikiImage = (filename: string) =>
  `https://oldschool.runescape.wiki/images/${filename.replaceAll(' ', '_')}?11111`
// Setup own cdn endpoint
export const getCdnImage = (filename: string) =>
  `https://tools.runescape.wiki/osrs-dps/cdn/${filename}`

export const keys = <T extends object>(o: T): (keyof T)[] =>
  Object.keys(o) as (keyof T)[]

export const typedMerge = <T, E extends PartialDeep<T>>(
  base: T,
  updates: E,
): T => merge({}, base, updates)
...
export const PotionMap: {
  [k in Potion]: {
    name: string
    order: number
    image: string
    calculateFn: (skills: PlayerSkills) => Partial<PlayerSkills>
  }
} = {
...
export const CombatStyleMap: {
  [k in EquipmentCategory]: { [k: string]: { image: string } }
} = {
...
export const getCombatStylesForCategory = (
  ...
  ret.push({ name: 'Spell', type: 'magic', stance: 'Manual Cast' })
  return ret
}
