...
const applyDefenceReductions = (m: Monster): Monster => {
  ...
  if (reductions.arclight > 0) {
    ...
    const arclightDivisor = m.attributes.includes(MonsterAttribute.DEMON)
      ? 10
      : 20
    m = newSkills(m, {
      atk:
        m.skills.atk -
        reductions.arclight *
          (Math.trunc(baseSkills.atk / arclightDivisor) + 1),
      str:
        m.skills.str -
        reductions.arclight *
          (Math.trunc(baseSkills.str / arclightDivisor) + 1),
      def:
        m.skills.def -
        reductions.arclight *
          (Math.trunc(baseSkills.def / arclightDivisor) + 1),
    })
  }
