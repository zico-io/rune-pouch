...
const applyCoxScaling = (m: Monster): Monster => {
  ...
  if (OLM_IDS.includes(m.id)) {
    const olmHp = () =>
      (OLM_HEAD_IDS.includes(m.id) ? 400 : 300) *
      (ps - Math.trunc(ps / 8) * 3 + 1)
    const olmDefence = (base: number) =>
      Math.trunc(
        (((base * (sqrt(ps - 1) + Math.trunc(((ps - 1) * 7) / 10) + 100)) /
          100) *
          (cm ? 3 : 2)) /
          2,
      )
    const olmOffence = (base: number) =>
      Math.trunc(
        (((base * (sqrt(ps - 1) * 7 + (ps - 1) + 100)) / 100) * (cm ? 3 : 2)) /
          2,
      )
    return {
      ...m,
  ...
  const scaleHp = (base: number) => {
    if (SCAVENGER_BEAST_IDS.includes(m.id)) {
      // no scaling
      return base
    }
  ...
  const scaleDefence = (base: number) => {
    ...
    return Math.trunc(
      (Math.trunc(
        (Math.trunc((base * (Math.trunc((hp * 4) / 9) + 55)) / 99) *
          (sqrt(ps - 1) + Math.trunc(((ps - 1) * 7) / 10) + 100)) /
          100,
      ) *
        (c ? f + 1 : f)) /
        f,
