  ...
  const options: EquipmentOption[] = useMemo(() => {
    ...
    for (const v of availableEquipment.filter((eq) => {
      if (
        ...
        eq.name.match(
          /(Fine mesh net|Wilderness champion amulet|\(Wilderness Wars)/,
        ) ||
    ...
    cross(blowpipeEntries, dartEntries).forEach(([blowpipe, dart]) => {
      const newStrength =
        blowpipe.equipment.bonuses.ranged_str +
        dart.equipment.bonuses.ranged_str
      entries.push({
        ...blowpipe,
    ...
    <ComboBox<EquipmentOption>
          ...
          <div className="basis-4 flex justify-center h-[20px] w-auto">
            {item.equipment.image && (
              <LazyImage
                ...
                src={
                  item.equipment.image
                    ? getCdnImage(`equipment/${item.equipment.image}`)
                    : undefined
                }
                alt=""
              />
      ...
      customFilter={(v) => {
        ...
        for (const eqOpt of v) {
          ...
          for (const [base, vars] of Object.entries(equipmentAliases)) {
            ...
            if (baseId === eqId || vars.includes(eqId)) {
              remainingVariantGroups[baseId] = remainingVariantGroups[baseId]
                ? [...remainingVariantGroups[baseId], eqId]
                : [eqId]
            }
          }
        }
        ...
        return v.filter((eqOpt) => {
          ...
          if (Object.keys(equipmentAliases).includes(eqId.toString()))
            return true
