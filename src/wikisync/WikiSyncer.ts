...
import {
  GetPlayerRequest,
  WikiSyncerRequestType,
  WikiSyncerResponsesUnion,
} from './WikiSyncerTypes'
...
export class WikiSyncer {
  ...
  private inFlightRequests = {
    getPlayer: new Map<number, InFlightRequest<ImportableData>>(),
  }
  ...
  private onMessage(message: MessageEvent) {
    ...
    switch (response._wsType) {
      ...
      case WikiSyncerRequestType.GET_PLAYER:
        if (response.error) {
          this.inFlightRequests.getPlayer
            .get(response.sequenceId)
  ...
  getPlayer() {
    const p = new Promise<ImportableData>((resolve, reject) => {
      if (this.ws) {
        ...
        this.inFlightRequests.getPlayer.set(this.nextSequenceID, {
          resolve,
          reject,
        })
